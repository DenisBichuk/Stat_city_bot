...
"🔸Не нужно скачивать никаких программ и прочих некомфортных действий", reply_markup=kb.tarifs_kb)
...
'❗️Вам будет выдан личный логин и пароль, благодаря такой системе входа вы не будете вылетать и полноценно сможете пользоваться сервисом.\n\n'
'После оплаты с вами свяжется менеджер и проведет проверку на подставного покупателя, это происходит быстро. 😊', reply_markup=kb.tarifs_kb)


@dp.message_handler(lambda message: message.text == "Оплатить и получить доступ✅")
async def prem_tarif(message: types.Message):
    photo = InputFile('payments.jpg')
    await bot.send_photo(chat_id=message.chat.id, photo=photo)
    await bot.send_message(reply_markup=kb.payments_kb)
