...
leo = KeyboardButton('Лео Шевченко')
pavel = KeyboardButton('Павел Шевченко')
elena = KeyboardButton('Елена Меньшина "Азбука Масштабирования"')
need_help = KeyboardButton('Обратится в поддержку✍')
start_kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=1).add(analitics, about, chat_and_reviews, leo, pavel, elena, need_help)

total_domination = KeyboardButton('Тотальное доминирование на МП🦾🔝')
to_the_lyam = KeyboardButton('TO THE ЛЯМ🍋')
main_menu = KeyboardButton('Главное меню🗂')
pavel_kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=1).add(total_domination, to_the_lyam, main_menu)

ozon = KeyboardButton('Лео Шевченко «Выход в Топ на OZON»📉')
top = KeyboardButton('В Топе на маркетплейс 11.0🔥')
main_menu = KeyboardButton('Главное меню🗂')
leo_kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=1).add(top, ozon, main_menu)
