...
about_sklad = KeyboardButton('Подробнее о складчине💬')
go_to_tp = KeyboardButton('Обратится в поддержку✍')
main_menu = KeyboardButton('Главное меню🗂')
back = KeyboardButton('Назад⬅')
tarifs_kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2).add(pay_and_get, about_sklad, go_to_tp, main_menu, back)

any_card = KeyboardButton('Любой картой✅')
tinkoff_sbor = KeyboardButton('Тинькофф Сбор☑')
main_menu = KeyboardButton('Главное меню🗂')
back = KeyboardButton('Назад⬅')
payments_kb = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2).add(any_card, tinkoff_sbor, main_menu, back)
